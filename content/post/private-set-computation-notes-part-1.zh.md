...
第 15 页已经完整实现了一次 OT：Alice 把明文消息 $m_0,m_1$ 分别加密成 $C_0,C_1$ 发给 Bob，Bob 算出自己所选分支的密钥并解出 $m_b$。这里“尚未得到 PSI 交集”不是说 OT 没有完成，而是说仅凭“二选一并解密”这个功能，还不能判断哪些集合元素相同。只有后续协议进一步规定 $m_0,m_1$ 如何承载集合匹配信息时，这次 OT 才成为 PSI 求交过程中的一步。
...
>
> **暂时不用：** RSA、盲签名、FHE、PHE和多项式判零。这里已经完成一次 OT 消息传输；若尚未规定消息与集合元素的对应关系，就还不能仅由 $m_b$ 得到 PSI 交集。
...
学到这里容易产生一个疑问：“Alice 已经有两个值，Bob 也选到并解开了一个，是不是交集已经算完？”更准确的答案是：**OT 已经完成，但是否得到交集取决于消息内容的定义。**

第 15 页完成的是一次安全二选一，Bob 能解出所选明文 $m_b$；第 16～21 页批量生成二选一所需的相关密钥。若当前只在讲通用 OT，$T,T',Q,r,s$ 都不是集合元素或交集结果。后续 OT-based PSI 会再规定这些 OT 消息怎样承载集合匹配信息。
...
$$\text{一次 Base OT 能让 Bob 解出所选明文；能否据此判断交集，取决于明文承载的含义}$$
